...
{	
	...
	function setupNetworkHandlers() {
		// CRITICAL: Double-check to prevent duplicate registration
		if (networkHandlersSetup) {
			console.warn('Network handlers already setup, skipping');
			return;
		}
	...
	function sendNetworkUpdates() {
		...
		// FIXED: Guard against uninitialized or invalid orbiters
		if (!orbiters || !Array.isArray(orbiters)) {
			console.warn('Orbiters not ready for network update');
			return;
		}
		...
		try {
			...
			// Send orbiter updates (NetworkManager handles throttling internally)
			networkManager.updateOrbiters(orbiters);
		} catch (error) {
			console.error('Error sending network updates:', error);
		}
	}
	...
	function render ()
	{
		...
		// FIXED: Remove double throttling - let NetworkManager handle it
		// NetworkManager already throttles at 50ms internally
		if (useWebSocket) {
			sendNetworkUpdates(); // Called every frame, NetworkManager throttles internally
		}
